...
class Database {
    ...
    async verifySchema(){
        ...
        */
        ...
        await this.query("CREATE TABLE IF NOT EXISTS identity_management_groups (uid INT, gid INT, updated TIMESTAMP, PRIMARY KEY(uid, gid))")
        ...
        */
        ...
        await this.query("CREATE TABLE IF NOT EXISTS identity_management_splits (uid INT, sid INT, updated TIMESTAMP, PRIMARY KEY(uid, sid))")
        ...
        */
        ...
        await this.query("CREATE TABLE IF NOT EXISTS events (eid INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, etyid INT, etid_used INT, name VARCHAR, begin TIMESTAMP, show BOOLEAN, data VARCHAR, updated TIMESTAMP, location VARCHAR, description VARCHAR)")
        ...
        */
        ...
        await this.query("CREATE TABLE IF NOT EXISTS groups (gid INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name VARCHAR, icon VARCHAR, description VARCHAR, uid_primary INT, extra_data VARCHAR, updated TIMESTAMP)")
        ...
        */
        ...
        await this.query("CREATE TABLE IF NOT EXISTS splits (sid INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, gid INT, name VARCHAR, icon VARCHAR, updated TIMESTAMP, uid_primary INT, extra_data VARCHAR)")
    ...
    async setSplit(sid, gid, name, icon, uid_primary, extra_data){
        ...
        if(existing_split.success && existing_split.data.length > 0){
            ...
            if(uid_primary == null) uid_primary = split.uid_primary
            if(extra_data == null) extra_data = split.extra_data

            return this.edit("UPDATE splits SET gid = " + gid + ", name = '" + name + "', icon = '" + icon + "', uid_primary = " + uid_primary + ", extra_data = '" + extra_data + "', updated = CURRENT_TIMESTAMP WHERE sid = " + sid)
        }else{
            return this.edit("INSERT INTO splits (gid, name, icon, uid_primary, extra_data, updated) VALUES (" + gid + ", '" + name + "', '', " + uid_primary + ", '', CURRENT_TIMESTAMP) RETURNING *")
        }
    }
    ...
    async setGroup(gid, name, icon, description, uid_primary, extra_data){
        ...
        if(existing_group.success && existing_group.data.length > 0){
            ...
            if(uid_primary == null) uid_primary = group.uid_primary
            if(extra_data == null) extra_data = group.extra_data

            return this.edit("UPDATE groups SET name = '" + name + "', icon = '" + icon + "', description = '" + description + "', uid_primary = " + uid_primary + ", extra_data = '" + extra_data + "', updated = CURRENT_TIMESTAMP WHERE gid = " + gid)
        }else{
            return this.edit("INSERT INTO groups (name, icon, description, uid_primary, extra_data, updated) VALUES ('" + name + "', '', '', " + uid_primary + ", '', CURRENT_TIMESTAMP) RETURNING *")
        }
    }
